...
class SecurityAudit {
  ...
  async checkSafeBrowsing(url) {
    if (!GOOGLE_SAFE_BROWSING_API_KEY) {
      return { warning: 'Google Safe Browsing API key not configured. Set GOOGLE_SAFE_BROWSING_API_KEY environment variable.' };
    }
  ...
  async scanCodeForMixedContent() {
    ...
    if (this.results.mixedContent.length === 0) {
      ...
      this.log('FAIL', 'Mixed Content', `Found ${this.results.mixedContent.length} mixed content issues`);
    }
  }
  ...
  async auditSecurityHeaders() {
    ...
    for (const check of headerChecks) {
      ...
      if (headerValue) {
        this.log('PASS', 'Security Headers', `${check.description} present: ${headerValue.substring(0, 100)}${headerValue.length > 100 ? '...' : ''}`);
  ...
  generateReport() {
    ...
    if (this.results.securityHeaders.length > 0) {
      ...
      this.results.securityHeaders.forEach(header => {
        ...
        const value = header.present ? (header.value.length > 50 ? header.value.substring(0, 50) + '...' : header.value) : 'Missing';
