...
class TestRunner {
  ...
  validateAndDisplayResults(data) {
    ...
    if (data.queryPerformance) {
      ...
      this.results.validation.queryCount =
        data.queryPerformance.totalQueries === TEST_QUERIES.length;
      if (!this.results.validation.queryCount) {
        this.error(
          `Query count mismatch: expected ${TEST_QUERIES.length}, got ${data.queryPerformance.totalQueries}`
        );
      }
    }
    ...
    if (data.embeddingMetadata) {
      ...
      this.results.validation.correctModel =
        data.embeddingMetadata.modelUsed === 'sentence-transformers/all-MiniLM-L6-v2';
      if (!this.results.validation.correctModel) {
        this.error(
          `Incorrect model: expected all-MiniLM-L6-v2, got ${data.embeddingMetadata.modelUsed}`
        );
      }

      // Validate embedding dimensions
      this.results.validation.correctDimensions =
        data.embeddingMetadata.embeddingDimensions === 384;
      if (!this.results.validation.correctDimensions) {
        this.error(
          `Incorrect embedding dimensions: expected 384, got ${data.embeddingMetadata.embeddingDimensions}`
        );
      }
    }
  ...
  generateReport() {
    ...
    const report = {
      ...
      performance: {
        totalTestTime:
          this.results.performance.aeoAnalysis + (this.results.healthCheck?.responseTime || 0),
        healthCheckTime: this.results.healthCheck?.responseTime,
        aeoAnalysisTime: this.results.performance.aeoAnalysis,
      },
      errors: this.results.errors,
      success:
        this.results.errors.length === 0 && this.results.healthCheck && this.results.aeoAnalysis,
    };
    ...
    this.log(
      `Health Check: ${report.results.healthCheck.passed ? '✅' : '❌'} (${report.results.healthCheck.responseTime}ms)`
    );
    this.log(
      `AEO Analysis: ${report.results.aeoAnalysis.passed ? '✅' : '❌'} (${report.results.aeoAnalysis.responseTime}ms)`
    );
