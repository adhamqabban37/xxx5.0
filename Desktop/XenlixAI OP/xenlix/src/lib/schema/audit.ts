 ...
 */
function calculateOverallScore(extraction: SchemaExtraction, blockAudits: Array<{ issues: string[]; score: number }>): number {
  ...
  let bonus = 0;
  const hasOrganization = extraction.detectedTypes.includes('Organization') || extraction.detectedTypes.includes('LocalBusiness');
  const hasWebSite = extraction.detectedTypes.includes('WebSite');
  const hasFAQ = extraction.detectedTypes.includes('FAQPage');
  const hasArticle = extraction.detectedTypes.includes('Article') || extraction.detectedTypes.includes('BlogPosting');
