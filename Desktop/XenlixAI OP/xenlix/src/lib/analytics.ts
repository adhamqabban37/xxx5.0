  ...
  private sendToAnalyticsProvider(eventData: any): void {
    ...
    if (typeof window !== 'undefined' && (window as any).gtag) {
      (window as any).gtag('event', eventData.event, {
        ...
        value: eventData.properties.analysis_time_ms
          ? Math.round(eventData.properties.analysis_time_ms / 1000)
          : undefined,
      });
    }
  ...
  getConversionMetrics(): any {
    ...
    try {
      ...
      return {
        ...
        successful_analyses: todayEvents.filter((e: any) => e.event === 'aeo_analysis_success')
          .length,
        failed_analyses: todayEvents.filter((e: any) => e.event === 'aeo_analysis_error').length,
        upsell_clicks: todayEvents.filter(
          (e: any) =>
            e.event === 'upsell_website_builder_click' || e.event === 'upsell_consultation_click'
        ).length,
        conversion_rate:
          todayEvents.filter((e: any) => e.event === 'aeo_form_submit').length > 0
            ? (
                (todayEvents.filter((e: any) => e.event === 'aeo_analysis_success').length /
                  todayEvents.filter((e: any) => e.event === 'aeo_form_submit').length) *
                100
              ).toFixed(2) + '%'
            : '0%',
      };
