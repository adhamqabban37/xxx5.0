...
export async function POST(request: NextRequest) {
  try {
    ...
    const { websiteUrl, businessName, businessDescription, industry, targetAudience, keyServices } =
      validation.data;
...
function generateFAQJsonLD(businessInfo: any, auditResults: any) {
  ...
  const questions: string[] =
    Array.isArray(auditResults?.suggestedQuestions) && auditResults.suggestedQuestions.length > 0
