...
function analyzeAEOOptimization(text: string, title: string, headings: string[]): any {
  ...
  const overallAeoScore = (questionIntentScore + answerReadinessScore + conversationalToneScore) / 3;
...
function extractPrimaryKeyword(title: string, textContent: string, keywordDensity: { [key: string]: number }): string {
  ...
  const titleKeywordInDensity = titleWords.find(word => 
    keywordDensity[word] && keywordDensity[word] > 1
  );
...
function analyzeAIEngineOptimization(text: string, title: string, headings: string[]): any {
  ...
  if (textLength > 1500) {
    ...
    googleRecommendations.push("Increase content length to 1500+ words for better Google AI visibility");
  }
  ...
  if (text.includes('according to') || text.includes('research shows') || text.includes('studies indicate')) {
    googleScore += 30;
  } else {
  ...
  if ((text.match(/\d+\./g)?.length || 0) >= 3) { // Numbered lists
    openAIScore += 25;
  } else {
  ...
  if (text.includes('consider') || text.includes('depending on') || text.includes('in some cases')) {
    anthropicScore += 30;
  } else {
    anthropicRecommendations.push("Include conditional statements and considerations");
  }
  
  if (text.includes('pros and cons') || text.includes('advantages') || text.includes('disadvantages')) {
    anthropicScore += 25;
  } else {
    anthropicRecommendations.push("Present balanced perspectives with pros and cons");
  }
  
  if (headings.some(h => h.toLowerCase().includes('conclusion') || h.toLowerCase().includes('summary'))) {
    anthropicScore += 20;
  } else {
  ...
  if (text.includes('2024') || text.includes('2023') || text.includes('recently') || text.includes('latest')) {
    perplexityScore += 30;
  } else {
...
function extractBusinessAddress($: any, url: string): { address?: string; city?: string; state?: string; country?: string; phone?: string } {
  const result: any = {};
  ...
  const addressSelectors = [
    ...
    '.address', '.location', '.contact-info', '.contact-address',
    '.business-address', '.company-address', '.office-address',
    '.street-address', '.postal-address', '.location-info',
    '.contact-details', '.address-block', '.location-block',
    
    // ID patterns
    '#address', '#location', '#contact', '#office-location',
    '#business-location', '#company-address',
    
    // Data attributes
    '[data-address]', '[data-location]', '[data-contact]',
    
    // Common footer patterns
    'footer .address', 'footer .location', 'footer .contact',
    '.footer-address', '.footer-location', '.footer-contact',
    
    // Contact page patterns
    '.contact-page .address', '.contact-section .address',
    '.contact-info-block', '.location-details'
  ];
  ...
  $('script[type="application/ld+json"]').each((_: number, script: any) => {
    try {
      ...
      const extractFromSchema = (obj: any) => {
        if (obj['@type'] && (obj['@type'].includes('LocalBusiness') || 
  ...
  if (!result.address) {
    for (const selector of addressSelectors) {
      ...
      if (element.length) {
        ...
        const isValidAddress = addressPatterns.some(pattern => pattern.test(text)) ||
  ...
  if (!result.phone) {
    const phoneSelectors = [
      '[href^="tel:"]',
      '.phone', '.telephone', '.tel', '.call',
      '#phone', '#telephone', '#tel',
      '.contact-phone', '.business-phone', '.office-phone',
      '[data-phone]', '[data-tel]'
    ];
  ...
  const metaPatterns = [
    { field: 'address', names: ['address', 'street-address', 'business:contact_data:street_address'] },
  ...
  if (!result.address) {
    ...
    for (const section of contentSections) {
      ...
      if (sectionText.length > 50) {
        ...
        const addressMatches = sectionText.match(/\d+\s+[A-Za-z\s]+(Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Drive|Dr|Lane|Ln|Way|Court|Ct)[^.]*?\d{5}(?:-\d{4})?/gi);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('ECONNREFUSED') || error.message.includes('ENOTFOUND')) {
        return NextResponse.json({ 
          ...
          message: 'The website appears to be down or unreachable. Please check the URL and try again.',
          code: 'NETWORK_ERROR'
        }, { status: 400 });
      }
      ...
      if (error.message.includes('timeout')) {
        return NextResponse.json({ 
          ...
          message: 'The website took too long to respond. Please try again or check if the website is accessible.',
          code: 'TIMEOUT_ERROR'
        }, { status: 408 });
      }
      ...
      if (error.message.includes('certificate') || error.message.includes('SSL')) {
        return NextResponse.json({ 
          ...
          message: 'There is an issue with the website\'s security certificate. Please verify the URL is correct.',
          code: 'SSL_ERROR'
        }, { status: 400 });
      }
      ...
      if (error.message.includes('429') || error.message.includes('rate limit')) {
        return NextResponse.json({ 
          error: 'Rate limited',
          message: 'The website is currently blocking our analysis requests. Please try again later.',
          code: 'RATE_LIMITED'
        }, { status: 429 });
      }
    }
    ...
    return NextResponse.json({ 
      ...
      message: 'We encountered an issue analyzing this website. Please try again or contact support if the problem persists.',
      code: 'ANALYSIS_ERROR',
