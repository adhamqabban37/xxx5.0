...
class PlatformMonitor {
  ...
  async getGoogleBusinessProfile(businessName: string, location?: string): Promise<BusinessProfileData> {
...
class AIEntityAnalyzer {
  ...
  async queryAIEngine(engine: 'ChatGPT' | 'Perplexity' | 'Bing Copilot', businessName: string): Promise<AIEntityDescription> {
    ...
    const selectedDescription = engineData.descriptions[Math.floor(Math.random() * engineData.descriptions.length)];
  ...
  async analyzeAllEngines(businessName: string): Promise<Record<string, string>> {
    const engines: Array<'ChatGPT' | 'Perplexity' | 'Bing Copilot'> = ['ChatGPT', 'Perplexity', 'Bing Copilot'];
...
class ReputationAlertGenerator {
  generateAlerts(
    ...
    const shortDescriptions = Object.entries(aiDescriptions).filter(
      ([engine, desc]) => desc.length < 50 || desc.includes('no information') || desc.includes('not found')
    );
    ...
    if (shortDescriptions.length > 0) {
      alerts.push({
        ...
        actionRequired: 'Improve online presence and content to help AI engines understand your business better',
        timestamp: new Date().toISOString()
      });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!url || !businessName) {
      return NextResponse.json(
        { error: 'URL and business name are required' },
        { status: 400 }
      );
    }
    ...
    const overallRatingChange = ratingChanges.reduce((sum, change) => sum + change.change, 0) / ratingChanges.length;
    const ratingChangeStr = overallRatingChange >= 0 
    ...
    const avgRating = businessProfiles.reduce((sum, p) => sum + p.rating, 0) / businessProfiles.length;
    const consistencyScore = entityConsistency.status === 'OK' ? 100 : entityConsistency.status === 'Minor Issues' ? 70 : 40;
    const reputationScore = Math.max(0, 100 - (alerts.filter(a => a.severity === 'high').length * 20) - (alerts.filter(a => a.severity === 'medium').length * 10));
    
    const overallHealth = Math.round((avgRating / 5 * 40) + (consistencyScore * 0.3) + (reputationScore * 0.3));
