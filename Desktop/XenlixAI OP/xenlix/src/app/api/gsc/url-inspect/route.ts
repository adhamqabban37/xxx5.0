...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Extract sitemap info (if available in future API versions)
      const sitemaps = (result?.indexStatusResult as any)?.sitemap || [];
      ...
      // Extract AMP info if available (if available in future API versions)
      let ampInfo = null;
      const ampResult = (result as any)?.ampResult;
      if (ampResult) {
        ampInfo = {
          verdict: ampResult.verdict,
          ampUrl: ampResult.ampUrl,
          issues: ampResult.issues || [],
        };
      }
          ...
          return NextResponse.json(
            {
              ...
              message:
                'Permission denied for URL inspection. The URL may not be under your verified property.',
            },
            { status: 403 }
          );
        }
