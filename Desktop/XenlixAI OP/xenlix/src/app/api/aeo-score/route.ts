...
import { huggingFaceClient, SimilarityMatrix, calculateSimilarityScore } from '@/lib/huggingface-client';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`AEO Score request for ${url} by ${session.user.email} with ${validQueries.length} queries`);

    console.log(`[${requestId}] Starting AEO analysis for URL: ${url} with ${validQueries.length} queries`);
    ...
    try {
      ...
      return NextResponse.json(
        { 
          ...
          message: 'Unable to extract content from the provided URL. Please check the URL and try again.',
          requestId,
          timestamp: new Date().toISOString()
        },
        { status: 400 }
    ...
    try {
      ...
      const contentResponse = await huggingFaceClient.generateEmbeddings(websiteContent.textContent);
    ...
    try {
      ...
      aeoAnalysis = computeAEOAnalysis(
        validQueries,
        websiteContent,
        similarityMatrix
      );
    ...
    return NextResponse.json({
      ...
      similarityMatrix: {
        ...
        coveragePercentage: calculateSimilarityScore(similarityMatrix.similarities).coveragePercentage,
...
function computeAEOAnalysis(
  ...
  const contentQuality = {
    ...
    avgContentLength: websiteContent.textContent.reduce((sum, content) => sum + content.length, 0) / websiteContent.textContent.length,
  };
  ...
  if (semanticScore < 40) {
    recommendations.push({
      ...
      description: 'Content has low semantic similarity to user queries. Improve keyword alignment and content relevance.',
    });
  }
  ...
  if (!contentQuality.hasHeadings) {
    recommendations.push({
      ...
      description: 'Use clear headings (H1-H6) to structure content for better answer engine understanding.',
    });
  }
  ...
  return {
    ...
    queryPerformance: {
      ...
      avgConfidence: Math.round((queryPerformance.reduce((sum, q) => sum + q.confidence, 0) / queries.length) * 1000) / 1000,
