...
export async function POST(req: NextRequest) {
  const { targetUrl, snippet, strategy = 'exact', threshold = 0.85 } = await req.json() as { targetUrl?: string; snippet?: string; strategy?: Strategy; threshold?: number }
  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json({ status: 'fail', error: `Fetch status ${res.status}` }, { status: 502 })
    }
    ...
    return NextResponse.json({ status: 'fail', error: 'Fetch error', detail: e?.message }, { status: 500 })
  }
}
