...
class AEOContentAnalyzer {
  ...
  private async performSemanticAnalysis(content: string, intents: string[]): Promise<{
  ...
  private generateAnswerSnippet(intent: string): string {
    const snippets: Record<string, string> = {
      'what services do you offer': 'We specialize in [SERVICE_TYPE] with [X] years of experience serving [LOCATION]. Our comprehensive services include [LIST_SERVICES] to meet all your [INDUSTRY] needs.',
      'how much does it cost': 'Our [SERVICE_TYPE] starts at $[PRICE] with free estimates available. We offer competitive pricing and flexible payment options to fit your budget.',
      'what are your hours': 'We are open [DAYS] from [TIME] to [TIME]. Emergency services available 24/7 for urgent [SERVICE_TYPE] needs.',
      'where are you located': 'Proudly serving [CITY], [STATE] and surrounding areas including [NEARBY_CITIES]. Our main location is at [ADDRESS] with convenient parking.',
      'how to contact you': 'Call us at [PHONE] or email [EMAIL] for immediate assistance. You can also request a free quote through our online form.',
    };

    return snippets[intent] || `We provide comprehensive ${intent.replace('what ', '').replace('how ', '')} information. Contact us at [PHONE] to learn more about our services.`;
  }
  ...
  private async generateLocalBusinessSchema(pages: PageAnalysis[]): Promise<LocalBusinessSchema> {
    ...
    const schema: LocalBusinessSchema = {
      ...
      ...(businessInfo.socialMedia && businessInfo.socialMedia.length > 0 && { sameAs: businessInfo.socialMedia }),
  ...
  private determineBusinessType(crawlData: any): string {
    const content = `${crawlData.title || ''} ${crawlData.meta_description || ''} ${crawlData.extracted_content || ''}`.toLowerCase();
  ...
  private async extractBusinessInfo(pages: PageAnalysis[]): Promise<any> {
    ...
    if (homePage) {
      ...
      const logo = images.find((img: any) => 
    ...
    for (const page of contactSources) {
      ...
      if (!info.address) {
        const addressMatch = content.match(/\d+\s+[A-Za-z\s]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Drive|Dr|Lane|Ln|Way|Circle|Cir|Court|Ct)[A-Za-z\s,]*\d{5}/);
        if (addressMatch) {
  ...
  private async extractFAQsFromPage(page: PageAnalysis): Promise<Array<{question: string; answer: string}>> {
    const content = page.crawlData.extracted_content || '';
  ...
  private generateOptimizedMeta(page: PageAnalysis): { title: string; description: string } {
    ...
    description = description.length < 140 ? description + ' Professional service guaranteed.' : description;
  ...
  private extractLocation(content: string): string {
    const locationMatch = content.match(/(?:in|serving|located)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)/);
  ...
  private extractBenefit(content: string): string {
    const benefits = ['Licensed & insured', 'Free estimates', '24/7 emergency service', 'Satisfaction guaranteed'];
  ...
  private async validateResults(
    ...
    if (!validationResults.localBusiness.isValid) {
      ...
      fixes.push(...validationResults.localBusiness.errors.map(err => `Fix LocalBusiness: ${err}`));
    }
    ...
    return {
      ...
      richResultsTestUrls: {
        ...
        testing: [`https://search.google.com/test/rich-results?url=${encodeURIComponent(pages[0]?.url || '')}`]
      }
    };
  }
