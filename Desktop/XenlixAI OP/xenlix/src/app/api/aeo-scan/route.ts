...
async function performAEOAudit(formData: AEOScanRequest) {
  ...
  if (!faqSchemaPresent) {
    weakPoints.push({
      ...
      severity: 'high',
      description:
        'Missing FAQ structured data prevents AI engines from understanding your common questions',
  ...
  if (!howToContentPresent) {
    weakPoints.push({
      ...
      severity: 'medium',
      description:
        'Your website lacks step-by-step guides that AI engines recommend to users seeking solutions.',
      impact:
        'Why it matters: When customers ask AI "how to solve X," your competitors with instructional content get recommended instead of you—losing you qualified leads who are actively seeking help.',
    });
  } else {
  ...
  weakPoints.push({
    ...
    severity: 'medium',
    description:
      "Your content isn't written in the conversational style that AI engines prefer when answering customer questions.",
    impact:
      'Why it matters: When customers ask AI for business recommendations, unclear messaging means you get overlooked for competitors who communicate more directly—resulting in lost opportunities worth thousands in potential revenue.',
  });
  ...
  return {
    ...
    aiReadinessLevel:
      baseScore < 40 ? 'Poor' : baseScore < 60 ? 'Fair' : baseScore < 80 ? 'Good' : 'Excellent',
  };
}
