...
function checkRateLimit(identifier: string, windowMs: number, maxRequests: number): RateLimitResult {
  const now = Date.now();
...
async function submitToIndexNow(urls: string[], retryCount = 0): Promise<{ success: boolean; response?: any; error?: string }> {
  const maxRetries = 3;
...
async function logSubmission(urls: string[], success: boolean, response?: any, error?: string, userId?: string) {
  const logEntry = {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const minuteLimit = checkRateLimit(`${userId}-minute`, 60 * 1000, RATE_LIMIT.MAX_REQUESTS_PER_MINUTE);
    ...
    const hourLimit = checkRateLimit(`${userId}-hour`, 60 * 60 * 1000, RATE_LIMIT.MAX_REQUESTS_PER_HOUR);
    ...
    const dayLimit = checkRateLimit(`${userId}-day`, 24 * 60 * 60 * 1000, RATE_LIMIT.MAX_REQUESTS_PER_DAY);
    if (!dayLimit.allowed) {
      return NextResponse.json(
        { 
