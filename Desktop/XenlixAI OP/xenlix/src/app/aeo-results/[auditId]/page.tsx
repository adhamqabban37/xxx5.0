...
export default function AEOResultsPage({ params }: { params: Promise<{ auditId: string }> }) {
  ...
  const generateRadarData = (overallScore: number) => {
    ...
    return {
      ...
      instructionalContent: Math.max(0, Math.min(100, baseScore + (Math.random() - 0.5) * variance)),
      brandAuthority: Math.max(0, Math.min(100, baseScore + (Math.random() - 0.5) * variance))
    };
  };

  // Calculate radar chart points
  const getRadarPoint = (value: number, angle: number, centerX: number, centerY: number, maxRadius: number) => {
  ...
  return (
    <div className="min-h-screen relative" style={{backgroundColor: '#0B1426'}}>
      ...
      <div 
        ...
        style={{
          background: 'radial-gradient(circle at 85% 85%, rgba(30, 58, 138, 0.4) 0%, rgba(59, 130, 246, 0.2) 35%, transparent 70%)'
        }}
      />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-xl p-6 mb-8 border border-slate-200/20 transition-all duration-300 hover:shadow-[0_15px_35px_rgba(0,0,0,0.2)]" 
             style={{
               boxShadow: '0 10px 25px rgba(0, 0, 0, 0.15), 0 4px 10px rgba(0, 0, 0, 0.1), 0 0 0 1px rgba(255, 255, 255, 0.05)'
             }}>
            ...
            <div className="lg:col-span-2">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-shrink-0">
                  ...
                  <div className={`w-16 h-16 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center ${audit.businessInfo.logoUrl ? 'hidden' : ''}`}>
                    <span className="text-white font-bold text-xl">
                      {audit.businessInfo.name && audit.businessInfo.name !== 'Unknown Business' && audit.businessInfo.name !== 'Business Analysis'
                        ? audit.businessInfo.name.charAt(0).toUpperCase()
                        : audit.websiteUrl.replace(/^https?:\/\//, '').replace(/^www\./, '').charAt(0).toUpperCase()
                      }
                    </span>
                  </div>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h3 className="text-xl font-bold text-gray-900 mb-2">
                    {audit.businessInfo.name !== 'Unknown Business' && audit.businessInfo.name !== 'Business Analysis' 
                      ? audit.businessInfo.name 
                      : `Business Analysis for ${audit.websiteUrl.replace(/^https?:\/\//, '').replace(/^www\./, '').split('/')[0]}`
                    }
                  </h3>
                  ...
                  {audit.businessInfo.description && audit.businessInfo.description !== `Business website: ${audit.websiteUrl}` && (
          ...
          {/* Social Profiles */}
          {audit.businessInfo.socialProfiles && Object.keys(audit.businessInfo.socialProfiles).length > 0 && (
            <div className="mt-6 pt-6 border-t border-gray-200">
              <h4 className="text-sm font-medium text-gray-700 mb-3">Social Presence</h4>
              <div className="flex flex-wrap gap-2">
                {Object.entries(audit.businessInfo.socialProfiles).map(([platform, url]) => (
                  url && (
                    <a
                      key={platform}
                      href={url}
        ...
        <div className="bg-white rounded-xl p-8 mb-8 border border-slate-200/20 transition-all duration-300 hover:shadow-[0_15px_35px_rgba(0,0,0,0.2)]"
             style={{
               boxShadow: '0 10px 25px rgba(0, 0, 0, 0.15), 0 4px 10px rgba(0, 0, 0, 0.1), 0 0 0 1px rgba(59, 130, 246, 0.1)'
             }}>
          <div className="text-center">
            <h2 className="text-3xl font-bold mb-2 text-white">Your AI Visibility Score</h2>
            <p className="text-slate-300 mb-8">How visible is your business when customers ask AI for recommendations?</p>
            ...
            <div className="relative w-64 h-32 mx-auto mb-6">
              ...
              <svg className="absolute inset-0 w-full h-full" viewBox="0 0 200 100">
                ...
                {/* Gradient definition */}
                <defs>
                  <linearGradient id={`scoreGradient-${audit.auditResults.overallScore}`} x1="0%" y1="0%" x2="100%" y2="0%">
                    <stop offset="0%" className={`${getScoreGradient(audit.auditResults.overallScore).split(' ')[0]?.replace('from-', 'stop-') || 'stop-gray-400'}`} />
                    <stop offset="100%" className={`${getScoreGradient(audit.auditResults.overallScore).split(' ')[1]?.replace('to-', 'stop-') || 'stop-gray-500'}`} />
                  </linearGradient>
                </defs>
              </svg>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-end pb-2">
                <div className={`text-5xl font-bold ${getScoreColor(audit.auditResults.overallScore)} mb-1`}>
                  {audit.auditResults.overallScore}
                </div>
                <div className={`text-lg font-semibold ${getScoreColor(audit.auditResults.overallScore)}`}>
              ...
              <div className="absolute left-1/4 -bottom-2 text-xs text-gray-500 font-medium">Fair</div>
              <div className="absolute right-1/4 -bottom-2 text-xs text-gray-500 font-medium">Good</div>
              <div className="absolute right-0 bottom-0 text-xs text-gray-500 font-medium">Excellent</div>
            </div>
            
            {/* Impact Statement */}
            <div className="bg-gray-50 border-l-4 border-gray-400 rounded-r-lg p-4 mb-6">
              <p className="text-gray-800 font-medium text-lg leading-relaxed">
                {getImpactStatement(audit.auditResults.aiReadinessLevel, audit.auditResults.overallScore)}
              </p>
            </div>
            
            {/* Context */}
            <p className="text-gray-600">
              Out of {audit.auditResults.maxScore} total points across ChatGPT, Claude, and Gemini visibility
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-8 mb-8 border border-slate-200/20 transition-all duration-300 hover:shadow-[0_15px_35px_rgba(0,0,0,0.2)]"
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">See exactly where you're strong and where you're losing potential customers</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
            ...
            <div className="relative">
              <svg viewBox="0 0 400 400" className="w-full h-80">
                ...
                {/* Axis Labels */}
                <text x="200" y="35" textAnchor="middle" className="text-sm font-medium fill-gray-700">
                  Technical Schema
                </text>
                <text x="375" y="205" textAnchor="middle" className="text-sm font-medium fill-gray-700">
                  Content Clarity
                </text>
                <text x="200" y="385" textAnchor="middle" className="text-sm font-medium fill-gray-700">
                  Instructional Content
                </text>
                <text x="25" y="205" textAnchor="middle" className="text-sm font-medium fill-gray-700">
                  Brand Authority
                </text>
                
                {/* Scale labels */}
                <text x="200" y="315" textAnchor="middle" className="text-xs fill-gray-500">60</text>
                <text x="200" y="275" textAnchor="middle" className="text-xs fill-gray-500">80</text>
                <text x="205" y="65" textAnchor="start" className="text-xs fill-gray-500">100</text>
              </svg>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-lg p-4 mt-6">
                ...
                <p className="text-sm text-gray-700 mb-3">
                  Reaching 100% on all metrics could increase your AI visibility by up to {100 - audit.auditResults.overallScore}%, 
                  potentially capturing <strong>{Math.round((100 - audit.auditResults.overallScore) * 0.3)}+ more customers daily</strong>.
                </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white rounded-xl p-6 border border-slate-200/20 transition-all duration-300 hover:shadow-[0_15px_35px_rgba(0,0,0,0.2)]"
               style={{
            ...
            <p className="text-gray-600 text-sm mb-6">
              These issues are causing you to lose potential customers to competitors who are better optimized for AI search.
            </p>
              ...
              {audit.auditResults.weakPoints.slice(0, 2).map((point, index) => (
                <div key={index} className="border-l-4 border-red-500 bg-red-50 rounded-r-lg p-4">
                  <div className="flex items-start justify-between mb-3">
                    <h4 className="font-semibold text-gray-900 text-base leading-tight">{point.category}</h4>
                    <span className={`px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap ml-3 ${getSeverityColor(point.severity)}`}>
                      {point.severity === 'high' ? 'HIGH IMPACT' : point.severity === 'medium' ? 'MEDIUM IMPACT' : 'LOW IMPACT'}
                    </span>
                  </div>
                  <p className="text-gray-700 text-sm mb-3 leading-relaxed">{point.description}</p>
                  <div className="bg-white border border-red-200 rounded-lg p-3">
                    <p className="text-red-800 text-sm font-medium leading-relaxed">{point.impact}</p>
                  </div>
                </div>
              ))}
              {audit.auditResults.weakPoints.length > 2 && (
                <div className="border-2 border-dashed border-orange-300 bg-orange-50 rounded-lg p-4 text-center">
                  ...
                  <p className="text-xs text-orange-600">
                    Get the complete analysis and step-by-step fixes to stop losing business to competitors
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="bg-white rounded-xl p-8 border-l-4 border-green-500 border border-slate-200/20 transition-all duration-300 hover:shadow-[0_15px_35px_rgba(0,0,0,0.2)]"
             style={{
               boxShadow: '0 10px 25px rgba(0, 0, 0, 0.15), 0 4px 10px rgba(0, 0, 0, 0.1)'
             }}>
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              ...
              <blockquote className="text-gray-700 text-base leading-relaxed mb-4">
                "My dental practice went from a <strong>52/100 to an 88/100</strong> in just 3 weeks using Xenlix's analysis. 
                The biggest shock? <strong>ChatGPT and Google AI started recommending my practice first</strong> when people asked about 
                'best dentist near me.' I'm now getting <strong>47% more new patients from AI search</strong> — that's an extra 
                <strong>$23,000 in monthly revenue</strong>. The $97 was the best investment I've made in years."
              </blockquote>
        ...
        <div className="bg-gradient-to-r from-red-600 to-orange-600 rounded-lg shadow-lg p-8 text-white text-center">
          <h3 className="text-3xl font-bold mb-4">Stop Losing {audit.auditResults.overallScore < 50 ? '$15,000+' : audit.auditResults.overallScore < 70 ? '$8,000+' : '$3,000+'} in Monthly Revenue</h3>
          <p className="text-lg mb-6 opacity-90">
            Transform your {audit.auditResults.overallScore}/100 score into a dominant AI search presence. 
            Get the exact roadmap to outrank competitors and capture the customers you're losing.
          </p>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 text-left">
            <div className="bg-white/10 rounded-lg p-4">
              <h4 className="font-semibold mb-2">💰 Revenue Recovery Blueprint</h4>
              <p className="text-sm opacity-90">Fix all {audit.auditResults.weakPoints.length} revenue leaks with priority-ranked action plan that competitors can't see</p>
            </div>
            <div className="bg-white/10 rounded-lg p-4">
              <h4 className="font-semibold mb-2">� 30-Day Domination Plan</h4>
              <p className="text-sm opacity-90">Copy-paste implementation guides to boost your score {audit.auditResults.overallScore < 50 ? '60+ points' : audit.auditResults.overallScore < 70 ? '40+ points' : '25+ points'} in 30 days</p>
            </div>
            <div className="bg-white/10 rounded-lg p-4">
              <h4 className="font-semibold mb-2">⚡ Instant AI Advantage Code</h4>
              <p className="text-sm opacity-90">Ready-to-upload schema markup that makes AI instantly understand and promote your business</p>
            </div>
          </div>
