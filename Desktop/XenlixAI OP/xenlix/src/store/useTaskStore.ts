...
export const useTaskStore = create<TaskState>((set, get) => ({
    ...
    set(state => {
      const tasks = state.tasks.map(t => {
        if (t.id === id) {
          ...
          const updated: Task = { ...t, status: nowCompleted ? 'completed' : 'pending', completedAt: nowCompleted ? new Date().toISOString() : null };
          // add event
          const evt: TaskEvent = {
            id: nanoid(),
            type: nowCompleted ? 'task_completed' : 'task_unchecked',
            timestamp: new Date().toISOString(),
            message: nowCompleted ? `Task '${t.title}' completed${t.impact ? ` - Score +${t.impact} pts` : ''}` : `Task '${t.title}' unchecked`,
            meta: { taskId: t.id, impact: t.impact }
          };
          const newScore = nowCompleted && t.impact ? state.overallScore + t.impact : (!nowCompleted && t.impact ? state.overallScore - t.impact : state.overallScore);
  ...
  simulateRescan: async () => {
    ...
    get().addEvent({ id: startId, type: 'rescan_started', timestamp: new Date().toISOString(), message: 'Re-scan initiated...', meta: {} });
    // fake delay
    ...
    const newTask: Task = {
      ...
      impact: 4,
      effort: 'Low',
  snippet: '<Image src="/hero.jpg" alt="Hero banner showing XenlixAI platform" width={1200} height={600} quality={80} priority sizes="(max-width: 600px) 100vw, 1200px" loading="eager" />',
      status: 'pending',
      createdAt: new Date().toISOString()
    };
    set(state => ({ tasks: [newTask, ...state.tasks] }));
    const endId = nanoid();
    get().addEvent({ id: endId, type: 'rescan_completed', timestamp: new Date().toISOString(), message: 'Re-scan complete – 1 new task detected (+4 pts potential).', meta: { added: 1 } });
  }
}));
