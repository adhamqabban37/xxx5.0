...
export const PageRow = ({
  ...
  <tr className="hover:bg-gray-50">
    ...
    <td className="px-6 py-4 text-sm text-gray-900">
      <IndexBadge status={indexStatus.status as any} verdict={indexStatus.verdict} />
    </td>
  </tr>
);
...
export const SummaryCard = ({
  ...
  <div className="bg-white p-6 rounded-lg border border-gray-200">
    ...
    {change && (
      <div
        className={`mt-2 text-sm ${change.type === 'increase' ? 'text-green-600' : 'text-red-600'}`}
      >
...
export const fetchUrlInspection = async (siteUrl: string, url: string) => {
  const response = await fetch(
    `/api/gsc/url-inspect?siteUrl=${encodeURIComponent(siteUrl)}&url=${encodeURIComponent(url)}`
  );
  const data = await response.json();
  return data.data;
};
