          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center flex-wrap gap-2">
                  ...
                  {typeof task.impact === 'number' && (
                    <span className="text-[10px] px-2 py-0.5 rounded bg-cyan-600/20 text-cyan-300 border border-cyan-600/40">+{task.impact} pts</span>
                  )}
                  {task.effort && (
                    <span className="text-[10px] px-2 py-0.5 rounded bg-slate-600/30 text-gray-300 border border-slate-600/40">{task.effort}</span>
                  )}
                </div>
                {task.description && (
              ...
              <div className="flex items-center gap-2">
                ...
                {task.snippet && (
                  ...
                  >
                    {copiedId === task.id ? <Check className="w-4 h-4 text-green-400" /> : <Copy className="w-4 h-4" />}
                  </button>
                )}
              </div>
            </div>
            {isOpen && task.snippet && (
              <div className="mt-3 bg-slate-900/80 border border-slate-700 rounded-md p-3 overflow-x-auto text-[11px] text-green-400 relative">
                <pre className="whitespace-pre-wrap"><code>{task.snippet}</code></pre>
              </div>
            )}
          </div>
        );
