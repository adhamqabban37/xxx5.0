...
console.log(
  `✅ Unique domains: ${uniqueDomains.length}/${enhancedSameAs.length} (${uniqueDomains.length === enhancedSameAs.length ? 'PASS' : 'FAIL'})`
);
...
console.log(
  `✅ sameAs count (5-8): ${countValid ? 'PASS' : 'FAIL'} (${enhancedSameAs.length} URLs)`
);
...
const authorityScores = [
  {
    url: 'https://www.linkedin.com/company/xenlixai',
    score: 95,
    reason: 'Essential B2B professional presence',
  },
  ...
  {
    url: 'https://www.crunchbase.com/organization/xenlixai',
    score: 80,
    reason: 'Business intelligence authority',
  },
...
const averageScore =
  authorityScores.reduce((sum, item) => sum + item.score, 0) / authorityScores.length;
console.log(`\n📈 Average Authority Score: ${averageScore.toFixed(1)}/100`);

console.log('\n🎯 Entity Linking Success Criteria:');
console.log(
  `✅ sameAs count ≥5: ${enhancedSameAs.length >= 5 ? 'PASS' : 'FAIL'} (${enhancedSameAs.length} URLs)`
);
console.log(`✅ All HTTPS: ${allHttps ? 'PASS' : 'FAIL'}`);
console.log(
  `✅ Unique domains: ${uniqueDomains.length === enhancedSameAs.length ? 'PASS' : 'FAIL'}`
);
