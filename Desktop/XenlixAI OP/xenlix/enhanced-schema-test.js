...
console.log(
  `✅ sameAs ≥3 URLs: ${hasSufficientSameAs ? 'PASS' : 'FAIL'} (${testConfig.sameAs.length} URLs)`
);
...
console.log(
  `✅ Valid ratings: ${hasValidRatings ? 'PASS' : 'FAIL'} (value: ${testConfig.aggregateRating.ratingValue}, count: ${testConfig.aggregateRating.reviewCount})`
);
...
console.log(
  `✅ Specific LocalBusiness subtype: ${hasSpecificSubtype ? 'PASS (ProfessionalService)' : 'FAIL'}`
);

// Validate parentOrganization reference
const hasParentOrganization =
  professionalService &&
  professionalService.parentOrganization &&
...
console.log(
  `📍 Address: ${napFields.address.addressLocality}, ${napFields.address.addressRegion}, ${napFields.address.addressCountry} ⚠️`
);
